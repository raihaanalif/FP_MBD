...
$check = pg_query($connect, "insert into products(product_id, product_name, unit_price, supplier_id, category_id, discontinued) 
                            values (nextval('sqc_products'), '$product_name', $unit_price, $ids, $idc, 0)");

if(!$check){
    echo "gagal memuat data";
}
else{
    header("location:product_page.php?create=berhasil");
}
?>